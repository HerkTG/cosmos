...
export class ForceManyBody<N extends InputNode, L extends InputLink> extends CoreModule<N, L> {
  ...
  public create (): void {
    ...
    for (let i = 0; i < store.pointsTextureSize * store.pointsTextureSize; ++i) {
      randomValuesState[i * 4] = Math.random() * 0.001
    }
  ...
  public initPrograms (): void {
    ...
    this.quadtreeCommand = reglInstance({
      frag: forceFrag(config.simulation?.collisionQuadtreeLevels ?? this.quadtreeLevels, this.quadtreeLevels),
      vert: updateVert,
      ...
      uniforms: {
        ...
        collision: () => config.simulation?.collision,
        theta: () => config.simulation?.collisionTheta,
        alpha: () => store.alpha,
